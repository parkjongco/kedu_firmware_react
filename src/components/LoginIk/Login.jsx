...
const Login = ({ setIsMypage }) => {
  ...
  const handleLogin = () => {
    ...
    axios.post(`http://192.168.1.11/auth`, auth)
      .then((resp) => {
  ...
  const handleLogout = () => {
    axios.post(`http://192.168.1.11/auth/logout`)
      .then(() => {
        console.log("로그아웃 성공");
  ...
  const handleMemberout = () => {
    ...
    axios.delete(`http://192.168.1.11/users`)
      .then(() => {
  ...
  const handleGoToHome = () => {
    window.location.href = "http://192.168.1.11:3000/"; // Directly navigate to the main homepage
  };
        ...
        {loginID ? (
          <>
            ...
            <div className={styles.actionButtons}>
              ...
              <button className={styles.actionButton} onClick={handleGoToHome}>메인 페이지로 이동</button>
            </div>
          </>
        ) : (
          <>
